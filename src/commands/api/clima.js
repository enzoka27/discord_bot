const { SlashCommandBuilder , EmbedBuilder , ActionRowBuilder , ButtonBuilder , ButtonStyle, ModalBuilder , TextInputBuilder, TextInputStyle} = require('discord.js'); // importa componentes da biblioteca 'discord.js'
...
module.exports = { //exporta o "data" e "execute", para ser usado em outros arquivos(como o index.js)
    ...
    data: new SlashCommandBuilder() //cria um novo comando e exporta ele para 'deploy-commands.js'
        ...
        .addStringOption(option => option // coloca opçao de escrever algo (obrigatorio nesse caso)
            .setName('cidade') // define o nome da opção
            .setDescription('Digite o nome da cidade à qual será mostrado o clima') // define sua descrição
    ...
    async execute(interaction){ // executa uma interação async (assincrona, ou seja, pode ter wait no meio e continuar rodando) e exporta tudo dentro dele para 'deplou-commands.js'
        ...
        const emojis ={ //criando object com os emojis que variam conforme o clima
            'Clear': '☀️',
            'Clouds': '☁️',
            'Rain': '🌧️',
            'Drizzle': '🌦️',
        ...
        try { //tenta pegar a opçao digita pelo usuario em 'cidade'
            ...
            async function mostrarClima(cidade) { // cria a função mostrarClima com a cidade como parametro
                ...
                embed = new EmbedBuilder() //cria novo card
                    ...
                    .setThumbnail(`https://openweathermap.org/img/wn/${clima.weather[0].icon}@2x.png`) // coloca uma imagem no canto superior direito (nesse caso é o icon do clima atual)
                    .addFields( //adiciona fields no card (seria um campo, para mostrar algo digitado)
                        ...
                        {name: `${emojis[clima.weather[0].main] || '🌡️'} Clima`, value: `${clima.weather[0].description.charAt(0).toUpperCase() + clima.weather[0].description.slice(1)}`}, // aqui ele coloca o emoji de acord com clima, e da capitalize no clima dado pela API
                        {name: '🌡️ Temperatura', value: `${clima.main.temp.toFixed(1)}°C`},
                        {name: '🤔 Sensação Térmica', value: `${clima.main.feels_like.toFixed(1)}°C`},
            ...
            await interaction.editReply({embeds: [embed] , components: [linha_botao]}) // mostrar o card com as informaões do cliam da cidade e o botao
            ...
            while(true) { // para sempre (até o usario nao clicar no botao em 30s ou nao mandar nada no modal em 15s)
                try{ // tenta pegar a cidade digitada no modal e atualizar o card
            ...
            if (erro.response && erro.response.status === 404){ // se for erro de cidade invalida 
                await interaction.editReply('❌ Cidade não encontrada! Verifique o nome e tente novamente.') // mostra 'cidade invalida', e usuario tem que usar novo comando (apenas se for a primeira interação)
            } else if (erro.code === 'InteractionCollectorError'){ // se for erro de ter ultrapassado o tempo limite de espera de interação
                ...
                if (houveErro){ // se teve erro na ultima mensagem atualizada
                        await interaction.editReply({embeds: [] , components: [linha_botao]}) //mostra a mensagem com o botao desativado, e sem o embed, e com a mensagem de erro
                } else { // se nao houve erro na ultima mensagem atualizada
                    await interaction.editReply({embeds: [embed] , components: [linha_botao]}) //mostra a mensagem com o botao desativado, e com o embed
                }

            } else { // se for qualquer outro erro
                await interaction.editReply('😵 Algo deu errado ao buscar o clima. Tente novamente!') // no discord mostra que deu erro ao tentar pegar a opçao de cidade
