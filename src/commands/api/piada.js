...
module.exports = {
  ...
  async execute(interaction) {
    // executa uma interação async (assincrona, ou seja, pode ter wait no meio e continuar rodando)
    ...
    try {
      //tenta buscar a api e mostrar na tela

      async function MensagemPiada() {
        //função para pegar api e fazer card(já que vai ser usada mais de uma vez, fica mais limpo com a função)
        ...
        embed = new EmbedBuilder() //cria novo card
          ...
          .addFields(
            //adiciona fields no card (seria um campo, para mostrar algo digitado, no caso temos 2)
      ...
      if (erro.code === "InteractionCollectorError") {
        //se for o erro que o usuario não clicou no botão ele roda
        if (botao) {
          //se o botao existir
        ...
        await interaction.editReply(
          "😵 Algo deu errado ao buscar a piada. Tente novamente!",
        ); //no discord avisa que não conseguiu buscar piada
      }
    }
  },
};
