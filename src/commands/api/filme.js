...
module.exports = { //exporta o "data" e "execute", para ser usado em outros arquivos(como o index.js)
    ...
    data: new SlashCommandBuilder() //cria um novo comando e exporta ele para 'deploy-commands.js'
        ...
        .addStringOption(option => option // coloca opçao de escrever algo (obrigatorio nesse caso)
            ...
            .setDescription('Digite o nome do filme à qual será mostrado as informações') // define sua descrição
    ...
    async execute(interaction){ // executa uma interação async (assincrona, ou seja, pode ter wait no meio e continuar rodando) e exporta tudo dentro dele para 'deplou-commands.js'
        ...
        try {
            ...
            async function InfoFilme(nome_filme) { // cria a função InfoFilme com o nome_filme como parametro

                const resposta = await axios.get(`http://www.omdbapi.com/?t=${encodeURIComponent(nome_filme)}&apikey=${process.env.OMDB_API_KEY}`) // request na API
                const filme_dados = resposta.data // coloca as informações do request da API, na variavel filme_dados

                if (filme_dados.Response === 'False') { // se for erro de não encontrar o filme
            ...
            await interaction.editReply({embeds: [embed] , components: [linha_botao]}) // mostrar o card com as informaões do filme e o botao
            ...
            while(true) { // para sempre (até o usario nao clicar no botao em 30s ou nao mandar nada no modal em 15s)
                try { // tenta pegar o filme digitado no modal e atualizar o card
            ...
            if (erro.message === 'Filme não encontrado'){ // se for erro de filme não encontrado 
                await interaction.editReply('❌ Filme não encontrado! Verifique o nome e tente novamente.') // mostra 'filme não encontrado', sem o embed antigo, mas com o botao(continua ativo para o usario digitar novo filme, volta pro inicio do loop)
                ...
                await interaction.editReply('😵 Algo deu errado ao buscar o filme. Tente novamente!') // no discord mostra que deu erro ao tentar pegar a opçao de filme
